    ...
    getAllUser: async (req, res) => {
        try {
            ...
            if (params.filter.firstName) {
                filter.firstName = { $regex: params.filter.firstName, $options: 'i' }
            }
            if (params.filter.lastName) {
                filter.lastName = { $regex: params.filter.lastName, $options: 'i' }
            }
            if (params.filter.email) {
                filter.email = { $regex: params.filter.email, $options: 'i' }
            }
            if (params.filter.phoneNumber) {
                filter.phoneNumber = { $regex: params.filter.phoneNumber, $options: 'i' }
            }
            filter.role = 'USER'
